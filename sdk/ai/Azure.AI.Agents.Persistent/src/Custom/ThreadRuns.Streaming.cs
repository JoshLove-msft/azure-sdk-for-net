...
#pragma warning restore AZC0015 // Unexpected client method return type.
        {
            ...
            CreateRunRequest createRunRequest = new(
                ...
                additionalMessages: options.AdditionalMessages?.ToList() as IList<ThreadMessageOptions> ?? new ChangeTrackingList<ThreadMessageOptions>(),
                overrideTools: options.OverrideTools?.ToList() as IList<ToolDefinition> ?? new ChangeTrackingList<ToolDefinition>(),
                ...
                metadata: (IDictionary<string, string>)options.Metadata?.ToDictionary(x => x.Key, x => x.Value) ?? new ChangeTrackingDictionary<string, string>(),
